    ...
    <div class="q-pa-sm box">
      ...
      >
        ...
        <template slot="header" slot-scope="props">
          ...
          <q-th key="num">班级人数</q-th>
          <q-th key="classteacher">限额</q-th>
        ...
        <template slot="body" slot-scope="props">
          <q-tr :props="props">
            ...
            <q-td key="num" :props="props">{{ props.row.studentIds.length }}</q-td>
            <q-td key="classteacher" :props="props">{{ props.row.studentLimit }}</q-td>
  ...
  watch: {
    serverData: function (value) {
      ...
      if (value[0] && value[0].deletedCount && value[0].deletedCount === 1) {
        ...
        this.$store.dispatch('getClassList', params)
      }
      this.$router.replace('/allClass')
      let v = value.data
      this.tableLoading = false
      this.done()
      let tableData1: Array<any> = []
      if (!v) return
      if (value.errCode === 0) {
        this.serverPagination.rowsNumber = v.count ? v.count : 0
        this.arr = v.data
        for (let i in this.arr) {
          const arr1:any = this.arr[i]
          tableData1[i] = arr1
        }
        // console.log(tableData1)
        this.tableData = JSON.parse(JSON.stringify(tableData1))
      }
      if (value.msg !== 'OK') {
