<template>
  <q-page class="">
    <div class="q-pa-sm q-mt-sm q-ml-sm ">
      <q-breadcrumbs>
        <q-breadcrumbs-el label="实验室列表" to="/alllabCourse"/>
      </q-breadcrumbs>
    </div>
      ...
      >
        <template slot="top-right" slot-scope="props">
          <q-search :props="props" style="fontSize: 0.9rem"
            hide-underline
            placeholder="输入实验室名称搜索"
            v-model="filter"
          />
        </template>
        <template slot="top-left" slot-scope="props">
          <q-btn :props="props" color="primary"  label="新增实验室" class="no-shadow" to="/addLab"/>
        </template>
        <template slot="header" slot-scope="props">
          <q-tr :props="props"  style="fontSize: 0.9rem">
          <q-th key="id" >序号</q-th>
          <q-th key="role">实验室名称名称</q-th>
          <q-th key="name">说明</q-th>
          <q-th key="status">实验室管理员</q-th>
          <!-- <q-th key="phoneNumber">联系电话</q-th>-->
          <q-th key="remark">实验室位置</q-th>
        ...
        <template slot="body" slot-scope="props">
          <q-tr :props="props">
            ...
            <q-td key="remark" :props="props">{{ props.row.description?props.row.description:'--'}}</q-td>
            <q-td key="teacherId" :props="props">{{ props.row.adminId?props.row.adminId.displayName:'--'}}</q-td>
            <q-td key="status" :props="props">{{ props.row.location?props.row.location:'--'}}</q-td>
...
export default Vue.extend({
  name: 'PageUser',
  computed: {
    ...mapGetters(['serverData', 'courseList', 'operateData', 'userInfo', 'checkedOpers', 'handleOpers'])
  },
  data () {
    return {
      enablePull: true,
  ...
  watch: {
    serverData: function (value) {
      ...
      if (value && value.deletedCount && value.deletedCount === 1) {
        ...
        this.$store.dispatch('getAllLab', params)
  ...
  methods: {
    ...
    editRow (val:string) {
      this.$router.push({ name: 'editLab', params: { rowData: val } })
    },
    ...
    checkRow (val: any) {
      ...
      // localStorage.setItem('courseName', val.courseName)
    ...
    delConfirm () {
      this.opened = false
      this.$store.dispatch('deleteLab', { id: this.deleteUser })
      // this.$socket.emit('deleteLab', { id: this.deleteUser })
    }
  }
})
</script>
