<template>
  <q-page class="">
    <div class="q-pa-sm q-mt-sm q-ml-sm ">
      <q-breadcrumbs>
        <q-breadcrumbs-el label="角色列表" to="/role"/>
      </q-breadcrumbs>
    </div>
    <div class="q-pa-sm box">
      ...
      >
        ...
        <!-- <template slot="top-left" slot-scope="props">
          <q-btn :props="props"  disabled=true color="primary"  label="新增角色" class="no-shadow"/>
        </template> -->
        <template slot="header" slot-scope="props">
          <q-tr :props="props"  style="fontSize: 0.9rem">
          <q-th key="id" >Id</q-th>
          <q-th key="role">角色名称</q-th>
        ...
        <template slot="body" slot-scope="props">
          <q-tr :props="props">
            ...
            <q-td key="roleName" :props="props">{{ props.row.roleType |  roleType}}</q-td>
...
export default Vue.extend({
  name: 'PageUser',
  computed: {
    ...mapGetters(['serverData', 'roleList', 'operateData', 'userInfo', 'checkedOpers', 'handleOpers'])
  },
  data () {
    return {
      enablePull: true,
  ...
  mounted () {
    ...
    axios.get(host + '/api/roles')
      .then(function (response) {
      })
      .catch(function (error) {
        console.log(error)
      })
    let fs = sessionStorage.getItem('fsa')
    if (fs && fs === 'true') {
      this.fullScreen = false
      // this.$refs.fsBtn.$el.click()
    }
  },
  beforeDestroy () {
  ...
  methods: {
    ...
    reqConditions (initPageIndex = true) {
      ...
      setTimeout(() => {
        ...
        // this.$socket.emit('getroles', params)
        this.$store.dispatch('getRoleList', params)
      }, 300)
      this.hideLoading()
    },
