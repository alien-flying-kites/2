    ...
    <div class="q-pa-sm box">
      ...
      >
        ...
        <template slot="top-left" slot-scope="props">
          <q-btn :props="props" color="primary"  label="添加排课" class="no-shadow" to="/addSchedule"/>
        </template>
        <template slot="header" slot-scope="props">
          <q-tr :props="props"  style="fontSize: 0.9rem">
          <q-th key="id" >序号</q-th>
          <q-th key="time">上课位置</q-th>
        ...
        <template slot="body" slot-scope="props">
          <q-tr :props="props">
            ...
            <q-td key="time" :props="props">{{ props.row.labId?props.row.labId.name:'--' }}</q-td>
            <q-td key="role" :props="props">{{ props.row.classId?props.row.classId.name:'-' }}</q-td>
...
export default Vue.extend({
  ...
  watch: {
    serverData: function (value) {
      ...
      if (value[0] && value[0].deletedCount && value[0].deletedCount === 1) {
        ...
        this.$store.dispatch('getScheduleList', params)
      }
      let v = value.data
  ...
  methods: {
    ...
    modifyRow (val: any) {
      ...
      this.$router.push({ name: 'editSchedule', params: { rowData: val } })
    },
    lessons (data) {
    ...
    delConfirm () {
      ...
      this.$store.dispatch('deleteSchedule', { id: this.deleteUser })
